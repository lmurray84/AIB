...
Invoke-WebRequest "https://software-download.microsoft.com/download/pr/19041.508.200905-1327.vb_release_svc_prod1_amd64fre_InboxApps.iso" -OutFile "$languagesDir\en-GB_Apps.iso"
...
# Copy language files
Robocopy "\\.\CDROM0\LocalExperiencePack\en-gb" "$languagesDir\LanguagePack" /256
Robocopy "\\.\CDROM0\x64\langpacks" "$languagesDir\LanguagePack" Microsoft-Windows-Client-Language-Pack_x64_en-gb.cab /256
...
foreach ($Appx in $AllAppx) {
    ...
    Start-Process -FilePath PowerShell -ArgumentList "Add-AppxProvisionedPackage -Online -PackagePath $pathappx -LicensePath $pathxml" -Wait -ErrorAction SilentlyContinue
    } else {
      
      Write-Host "Handeling without xml $appname"
      
      Start-Process -FilePath PowerShell -ArgumentList "Add-AppxProvisionedPackage -Online -PackagePath $pathappx -skiplicense" -Wait -ErrorAction SilentlyContinue
    }
}
